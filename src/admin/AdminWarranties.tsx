...
export default function AdminWarranties() {
  ...
  async function handleRegStatusChange(id: string, currentStatus: string) {
  ...
  async function handleClaimStatusChange(id: string, currentStatus: string) {
    const newStatus = claimStatusCycle[currentStatus] || "open";
    setUpdatingClaimId(id);

    try {
      await updateWarrantyClaimStatus(id, newStatus);
      setClaims((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, claim_status: newStatus } : c
        )
      );
      toast.success(`Status changed to ${newStatus}`);
    } catch {
                        ...
                        filteredRegistrations.map((r) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <button
                                onClick={() =>
                                  handleRegStatusChange(r.id, r.warranty_status)
                                }
                        ...
                        filteredClaims.map((c) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <button
                                onClick={() =>
                                  handleClaimStatusChange(c.id, c.claim_status)
                                }
